...
HUE_THRESHOLD = ([4, 176], [40, 80], [110, 130], [20, 30])
"""-----------------------------------------------------"""
...
class libCAMERA(object):
    ...
    def initial_setting(self, cam0port=2, cam1port=1, capnum=1):
    ...
    def image_show(self, frame0, frame1=None):
        if frame1 is None:
            ...
            cv2.imshow('line', frame0)
            cv2.imshow('obj', frame1)
    ...
    def object_detection(self, img, sample=0, mode="circle", print_enable=True):
        global h_cond
        result = None
        while(True):
            replica = img.copy()
